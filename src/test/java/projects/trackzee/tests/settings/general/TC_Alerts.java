...
public class TC_Alerts extends TestBase implements InitializePages {
	@BeforeClass()
	...
	public void testTotalRecordsPresenceBasedOnTheAppliedCompanyFilter(String[] excelData,
		...
		String branch = excelData[1];
		String alertType = excelData[2];
		String excpectedTotalRecords = excelData[4];
		if (iteration == 1) {
		...
		if (isCompanyPresent) {
			getFilterPage().selectBranchCommon(branch);
			getFilterPage().selectAlertTypeCommon(alertType);
		...
		getCommonPage().writeDataInExcelSheet("Alerts", "Alerts", iteration, 3, 5, totalRecords,
